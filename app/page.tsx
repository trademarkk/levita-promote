...
const modules = [
  ...
  { number: "06", title: "Три способа заявки", time: "10 мин" },
...
function ModuleContent({ active, onScenarioAnswered }: { active: number; onScenarioAnswered: (title: string) => void }) {
  ...
  if (active === 5) {
    return (
      <section className="content-section narrow">
        <div className="section-heading"><span className="eyebrow">Шаг 5</span><h1>Три способа оставить заявку</h1><p>Выбирай способ по ситуации: клиентка может записаться сразу, сделать это позже или отправить готовое SMS. Твоя задача - спокойно объяснить следующий шаг и оставить выбор за ней.</p></div>
        ...
        <ScriptCard label="Фраза для перехода">«Есть три удобных варианта: можно записаться через официальный VK-бот, отправить SMS по моему QR-коду сразу администратору или взять листовку с нашими данными и самостоятельно оставить заявку. Как будет удобнее?»</ScriptCard>
