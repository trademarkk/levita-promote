...
const difficultSituations = [
  ...
  },
  {
    ...
    wrongFeedback: "Не бери телефон и не вводи данные за клиентку: ей важно самой видеть, куда она отправляет свои данные.",
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      "«В VK-боте можно выбрать Мачуги, 4 или Ставропольскую, 174/1. Если удобнее другой адрес, это можно уточнить у администратора»",
    ],
    correct: 2,
    feedback: "Не собираем лишние данные и не определяем студию самостоятельно.",
    wrongFeedback: "Не проси адрес и не решай за клиентку, какая студия ей подходит. Пусть выберет сама или уточнит у администратора.",
  },
  {
    ...
    wrongFeedback: "Нельзя обещать, что данные «никуда не попадут», или просить прислать номер тебе. Объясни официальный способ записи.",
  },
  {
...
const testBank: TestQuestion[] = [
  ...
  { id: "certificate", critical: true, question: "Что даёт сертификат промоутера?", options: ["Два бесплатных групповых занятия", "Месячный абонемент", "Любую тренировку навсегда", "Скидку по выбору промоутера"], correct: 0, explanation: "По этому сертификату для клиентки — 2 бесплатных групповых занятия." },
  ...
  { id: "valid", critical: true, question: "Когда оплачивается лид?", options: ["Клиентка только открыла QR-код", "Клиентка взяла листовку", "До клиентки дозвонились, и она понимает, что такое LEVITA и почему ей звонят", "Клиентка купила любой абонемент"], correct: 2, explanation: "Оплата за лид начисляется, когда до клиентки дозвонились и она понимает, что такое LEVITA и почему ей звонят." },
  ...
  { id: "studio", question: "Где клиентка выбирает удобную студию?", options: ["В официальном VK-боте LEVITA", "В бумажном опроснике", "В телефоне промоутера", "После покупки абонемента"], correct: 0, explanation: "В боте доступны наши студии на Мачуги, 4 и Ставропольской, 174/1. Если клиентке удобнее другой адрес, это уточняет администратор." },
  ...
  { id: "trainer", question: "Как честно описать работу тренера?", options: ["Занятия ведут профессиональные тренеры и помогают каждой клиентке", "Тренер только включает музыку и не вмешивается", "Все клиентки занимаются полностью самостоятельно", "Можно обещать любой результат за одно занятие"], correct: 0, explanation: "В LEVITA занятия проходят с профессиональным тренером и индивидуальным вниманием." },
  ...
  { id: "pay", question: "За что промоутеру начисляется оплата?", options: ["За каждый разговор на улице", "За лид, которому дозвонились и который понимает LEVITA, а также за посещение им пробного занятия", "За количество выданных листовок", "Только за сканирование QR-кода"], correct: 1, explanation: "Оплата идёт отдельно за лид, которому дозвонились и который понимает, что такое LEVITA и почему ему звонят, а также за посещение этим клиентом пробного занятия." },
  ...
  { id: "survey-intro", critical: true, question: "Как честно начать соцопрос?", options: ["Скрыть название клуба до конца", "Назвать себя, LEVITA, около двух минут и пять коротких вопросов", "Сразу спросить телефон", "Сказать, что это государственный опрос"], correct: 1, explanation: "Сразу представься, назови LEVITA и скажи: около двух минут и пять коротких вопросов." },
    ...
    <article className="scenario">
      ...
      {selected !== undefined && (
        <p className={`scenario-feedback ${selected === correct ? "correct" : "wrong"}`}>
          <strong>{selected === correct ? "Почему это работает:" : "Почему этот ответ не подходит:"}</strong> {selected === correct ? feedback : (wrongFeedback ?? "Он может запутать клиентку, нарушить границы или создать неверное ожидание.")}
        </p>
      )}
    </article>
  );
      ...
      <>
        ...
        <section className="content-section">
          ...
          <div className="benefit-grid">
            ...
            <article><span>02</span><h3>Понятная оплата</h3><p>Оплата идёт отдельно: за лид, которому дозвонились и который понимает, что такое LEVITA и почему ему звонят, а также за посещение им пробного занятия.</p></article>
  ...
  if (active === 5) {
    return (
      <section className="content-section narrow">
        ...
        <div className="lead-methods">
          <article className="lead-method">
            <span className="eyebrow">Вариант 1 · на потом</span><h2>Листовка</h2>
            <p>Отдай её только после интереса. На листовке есть телефоны студий и QR-коды, чтобы клиентка могла спокойно разобраться и записаться позже.</p>
            <img src="/promoter-leaflet.png" alt="Листовка LEVITA с телефонами и QR-кодами" />
            <small>Если человек не готов решать сейчас, листовка сохраняет информацию без давления.</small>
          </article>
          <article className="lead-method featured">
            <span className="eyebrow">Вариант 2 · самостоятельно</span><h2>Официальный VK-бот</h2>
            <p>Покажи свой личный QR-код. Клиентка сама открывает бот, выбирает удобную студию и проходит запись на своём телефоне.</p>
            <ul className="studio-list">{OUR_STUDIOS.map((studio) => <li key={studio}>{studio}</li>)}</ul>
            <small>Если клиентке удобнее другой адрес LEVITA, скажи, что администратор уточнит варианты партнёрских студий. Подсказывай, куда нажать, но не забирай телефон и не переписывай данные.</small>
          </article>
          <article className="lead-method">
            <span className="eyebrow">Вариант 3 · SMS</span><h2>Личный QR-скриншот</h2>
            <p>На скриншоте два QR-кода: для iPhone и Android. После сканирования откроется SMS с готовым текстом «Я получила сертификат на 2 занятия. Промоутер ИМЯ» и номером администратора.</p>
            <img src="/promoter-sms-qr.png" alt="QR-коды для отправки SMS администратору LEVITA" />
            <small>Клиентке останется нажать «Отправить». QR-скриншот выдают тебе на весь период работы с LEVITA.</small>
          </article>
          <article className="lead-method max-method">
            <span className="eyebrow">Вариант 4 · MAX</span><h2>Официальный MAX-бот</h2>
            <p>Покажи свой личный QR-код на чат-бот LEVITA в MAX. Клиентка открывает официальный бот, знакомится с информацией и оставляет контакты самостоятельно.</p>
            <img src="/max-bot-example.jpg" alt="Пример официального MAX-бота LEVITA" />
            <small>Личный QR-код на MAX-бот выдают тебе на весь период работы. Не вводи и не переписывай данные клиентки сам.</small>
          </article>
        </div>
