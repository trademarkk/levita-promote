...
const dialogues: Dialogue[] = [
  {
    ...
    messages: [
      ...
      { role: "promoter", text: "Тогда вам может понравиться LEVITA. Это женский фитнес-клуб, у нас есть и спокойные, и активные направления. Для клиенток от промоутера первое пробное занятие бесплатно." },
  ...
  },
  {
    ...
    messages: [
      ...
      { role: "promoter", text: "Пожалуйста! Для клиенток от промоутера первое пробное бесплатно. Хорошего дня!" },
    ],
  ...
  },
  {
    ...
    outcome: "fail",
    messages: [
      { role: "promoter", text: "Девушка, стойте! У нас бесплатное занятие, берите листовку и оставляйте номер." },
...
const surveyDialogues: Dialogue[] = [
  {
    ...
    messages: [
      ...
      { role: "promoter", text: "Хотели бы бесплатно попробовать подходящее направление в ближайшее время?" },
...
const difficultSituations = [
  {
    title: "Просят назвать стоимость пробного",
    situation: "«А пробное точно бесплатное?»",
    options: [
      "«Для клиенток, которых приглашает промоутер, первое пробное бесплатно. Остальные условия уточнит администратор»",
      "«Наверное, но я не уверена»",
      "«Сначала оставьте номер, потом узнаете»",
    ],
    correct: 0,
    feedback: "Говорим только подтверждённое правило: первое пробное для лида от промоутера бесплатно.",
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      "«Здорово! Если захотите попробовать новое направление или атмосферу, первое пробное от промоутера бесплатно»",
...
const testBank: TestQuestion[] = [
  ...
  { id: "free", critical: true, question: "Что получает клиентка от промоутера?", options: ["Первое пробное занятие от промоутера бесплатно", "Месячный абонемент бесплатно", "Любую тренировку навсегда", "Скидку по выбору промоутера"], correct: 0, explanation: "Первое пробное для клиентки от промоутера бесплатно." },
...
function ModuleContent({ active, onScenarioAnswered }: { active: number; onScenarioAnswered: (title: string) => void }) {
    ...
    return (
      <>
        <section className="hero-grid">
          <div className="hero-copy">
            ...
            <div className="hero-facts">
              ...
              <div><strong>Бесплатно</strong><span>первое пробное для лида от промоутера</span></div>
        ...
        <section className="content-section">
          ...
          <div className="statement-card"><p>LEVITA — <Mark>женский фитнес-клуб</Mark> с активными и спокойными направлениями: силовыми тренировками, пилатесом, растяжкой, танцами и хореографией. Тебе не нужно продавать абонемент. Нужно заинтересовать, показать подходящий следующий шаг и понятно объяснить, что будет дальше.</p></div>
  ...
  if (active === 2) {
    return (
      <section className="content-section narrow">
        <div className="section-heading"><span className="eyebrow">Шаг 2</span><h1>Как начать разговор без давления</h1><p>Твоя цель — не выдать весь текст за десять секунд. Сначала создай спокойный контакт и пойми, интересно ли человеку продолжать.</p></div>
        ...
        <ScriptCard label="Вариант 2 · через вопрос">«Здравствуйте! Можно один короткий вопрос про спорт? Вы раньше занимались фитнесом, растяжкой или танцами?»<br /><br />После ответа: «Тогда вам может понравиться LEVITA. У нас есть и спокойные, и активные направления, а для клиенток от промоутера первое пробное занятие бесплатно».</ScriptCard>
  ...
  if (active === 4) {
    return (
      <section className="content-section narrow">
        <div className="section-heading"><span className="eyebrow">Шаг 4 · отдельная механика</span><h1>Социальный опрос на улице</h1><p>Опрос помогает начать спокойный разговор и понять интерес клиентки. Это не уловка: сразу назови себя, LEVITA, тему и длительность.</p></div>
        ...
        <div className="survey-questions">
          ...
          {[
            ...
            ["05", "Хотели бы бесплатно попробовать подходящее направление в ближайшее время?", "Да · Возможно · Нет"],
        ...
        <div className="survey-decision">
          <article className="yes"><span>Есть интерес</span><h3>Предложи запись</h3><p>Клиентка ответила «да» или «возможно» на последний вопрос, хочет начать, называет интересное направление или задаёт вопросы о клубе.</p><blockquote>«Спасибо! По вашим ответам вам может подойти … Хотите записаться на бесплатное пробное через наш VK-бот?»</blockquote></article>
  ...
  if (active === 7) {
    return (
      <section className="content-section narrow">
        <div className="section-heading"><span className="eyebrow">Шаг 7</span><h1>Первая смена без суеты</h1><p>Доступные смены публикуются в рабочем чате. Обычная продолжительность — 2–3 часа. Выбирай только тот слот, который точно сможешь отработать.</p></div>
