...
fact_cards = Table(
    [[
        ...
        card("Бесплатно", "Первое пробное для клиентки от промоутера.", WHITE),
...
story.append(numbered_rows([
    ...
    ("Коротко расскажи", "LEVITA - женский фитнес-клуб с активными и спокойными направлениями. Первое пробное от промоутера бесплатно."),
...
story += [
    ...
    numbered_rows([
        ...
        ("Готовность", "Хотели бы бесплатно попробовать подходящее направление в ближайшее время? Да / возможно / нет."),
    ]),
    Spacer(1, 4 * mm),
]

survey_cards = Table([[ 
    card("Есть интерес", "«Спасибо! По вашим ответам вам может подойти ... Если вам удобно, можно перейти к записи через наш VK-бот»", GOLD_SOFT, 85 * mm),
...
answers = [
    ...
    [p("«Я уже занимаюсь»", card_title), p("«Здорово! Если захотите попробовать новое направление, первое пробное от промоутера бесплатно».", body_small)],
