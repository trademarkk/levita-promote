...
story += [banner("КАК ОПЛАЧИВАЕТСЯ РАБОТА", "Оплата идёт отдельно за лид, которому удалось дозвониться и который понимает, что такое LEVITA и почему ему звонят. Отдельная оплата - за посещение этим клиентом пробного занятия.")]
...
survey_cards = Table([[ 
    card("Есть интерес", "«Спасибо! Ваши ответы очень ценны для нас. За прохождение опроса мы дарим вам сертификат на 2 бесплатных групповых занятия! Давайте сразу запишемся?»", GOLD_SOFT, 85 * mm),
...
story += [
    p("Три способа оставить заявку", title),
    p("Выбирай способ по ситуации: клиентка может записаться сразу, сделать это позже или отправить готовое SMS. QR-скриншот выдают тебе на весь период работы с LEVITA.", subtitle),
    ...
    numbered_rows([
        ...
        ("Выберите студию", "Клиентка сама выбирает любой удобный адрес LEVITA в Краснодаре."),
...
story += [
    p("Три способа заявки: выбор по ситуации", section),
    p("1. Листовка - если клиентка хочет взять информацию с собой и вернуться позже. 2. Официальный VK-бот - если ей удобно записаться самостоятельно сейчас. 3. Личный SMS-скриншот - если ей проще отправить готовое сообщение администратору.", body),
    Spacer(1, 2 * mm),
    p("Адреса LEVITA в Краснодаре", section),
    p("ул. им. В. Н. Мачуги, 4<br/>ул. Ставропольская, 174/1<br/>ул. Красная, 139<br/>ул. Кореновская, 2<br/>ул. им. П. Метальникова, 42<br/>ул. Мира, 1<br/>ул. 1 Мая, 196/1", body),
    Spacer(1, 3 * mm),
    banner("ЛИЧНЫЙ SMS-СКРИНШОТ", "На нём два QR-кода - для iPhone и Android. После сканирования открывается SMS с текстом «Я получила сертификат на 2 занятия. Промоутер ИМЯ» и номером администратора. Клиентке остаётся нажать «Отправить». QR-скриншот выдают тебе на весь период работы."),
    PageBreak(),
]
