...
def page_decor(canvas, doc):
    ...
    if doc.page == 3:
        ...
        canvas.drawCentredString(
            width / 2,
            16.8 * mm,
            "Данные: не копируй из VK-бота  •  Безопасность: отойди и напиши куратору  •  Тест: 27/30, две попытки",
        )
    canvas.restoreState()
...
fact_cards = Table(
    [[
        ...
        card("2 занятия", "По сертификату промоутера.", WHITE),
...
story += [banner("КАК ОПЛАЧИВАЕТСЯ РАБОТА", "Оплата идёт отдельно за валидный лид: администратор дозвонился, клиентка ответила и понимает, кто и зачем звонит. Отдельная оплата - за посещение клиенткой пробного занятия.")]
story.append(numbered_rows([
    ...
    ("Коротко расскажи", "LEVITA - женский фитнес-клуб с активными и спокойными направлениями. По этому сертификату для Вас - 2 групповых занятия."),
...
story += [
    ...
    p("Честно назови себя и LEVITA. Скажи, что это пять вопросов примерно на две минуты. На листе отмечай только варианты ответов - без имени, телефона и других персональных данных.", subtitle),
    banner("ФРАЗА ДЛЯ СТАРТА", "«Здравствуйте! Меня зовут ..., я представляю женский фитнес-клуб LEVITA. Мы проводим короткий опрос о спорте - пять вопросов, около двух минут. Можно?»"),
    Spacer(1, 4 * mm),
    ...
    numbered_rows([
        ...
        ("Готовность", "Хотели бы получить сертификат на 2 групповых занятия и попробовать подходящее направление в ближайшее время? Да / возможно / нет."),
    ]),
    Spacer(1, 4 * mm),
]

survey_cards = Table([[ 
    card("Есть интерес", "«Спасибо! По вашим ответам Вам может подойти ... По этому сертификату для Вас - 2 групповых занятия. Можно выбрать удобный способ записи»", GOLD_SOFT, 85 * mm),
...
story += [
    p("VK-бот, листовка и смена", title),
    p("VK-бот - один из способов самостоятельной записи. Если клиентке он удобен, она переходит по твоему персональному QR со своего телефона. Ты не видишь, не записываешь и не фотографируешь её данные.", subtitle),
    ...
    numbered_rows([
        ...
        ("Откройте бота", "Клиентка нажимает «Записаться на пробное» в официальном VK-боте LEVITA."),
        ("Выберите студию", "Клиентка сама выбирает Ставропольскую, 174/1 или Мачуги, 4."),
...
answers = [
    ...
    [p("«Я уже занимаюсь»", card_title), p("«Здорово! Если захотите попробовать новое направление, для Вас есть сертификат на 2 групповых занятия».", body_small)],
