...
test("keeps the business and test rules in the source", async () => {
  ...
  assert.match(page, /Бывшая клиентка считается оплачиваемым контактом/);
  ...
  assert.match(page, /Нажмите «Записаться»/);
  assert.doesNotMatch(page, /Нажмите «Записаться на пробное»/);
  assert.match(page, /Вот сертификат, чтобы информация осталась у вас/);
  assert.match(page, /через наш официальный бот в VK или MAX/);
  assert.match(page, /Возьми листы опроса и сертификаты/);
  assert.match(page, /Нашему самому возрастному клиенту — 78 лет/);
  assert.doesNotMatch(page, /lead-form-example\.png|личной контакт-форме/i);
